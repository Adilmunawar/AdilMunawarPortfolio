            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="text-gray-300 hover:text-cyber-cyan px-3 py-2 text-sm font-medium transition-colors duration-300 relative group"
                >
                  {item.name}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-black/80 backdrop-blur-lg">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-gray-300 hover:text-cyber-cyan block px-3 py-2 text-base font-medium transition-colors duration-300"
