...
const ContactSection = () => {
  ...
  const contactMethods = [{
    ...
    hoverColor: 'hover:from-red-500/40 hover:to-pink-500/40',
    ...
    hoverColor: 'hover:from-green-500/40 hover:to-emerald-500/40',
    ...
    hoverColor: 'hover:from-pink-500/40 hover:to-purple-500/40',
    ...
    hoverColor: 'hover:from-blue-600/40 hover:to-indigo-500/40',
  ...
  return (
    <section id="contact" className="min-h-screen py-20 px-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-r from-cyber-cyan/10 to-blue-500/10 rounded-full blur-3xl animate-drift-1"></div>
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-full blur-3xl animate-drift-2"></div>
          ...
          {contactMethods.map((method, index) => (
            ...
            >
              ...
              <div className="relative text-center transform group-hover:translate-y-1 transition-transform duration-500">
                ...
                <div className={`w-20 h-20 rounded-2xl border-3 ${method.borderColor} bg-gradient-to-br ${method.bgColor} flex items-center justify-center mx-auto mb-6 group-hover:scale-125 group-hover:rotate-12 transition-all duration-700 backdrop-blur-sm shadow-xl ${method.glowColor} ${hoveredCard === index ? 'animate-bounce' : ''}`}>
                  <method.Icon size={36} className={`${method.color} group-hover:drop-shadow-lg transition-all duration-500`} />
                </div>
                
                {/* Enhanced text content */}
                <h3 className="font-bold text-xl text-white mb-2 group-hover:text-cyber-cyan transition-colors duration-500">{method.label}</h3>
                <p className="text-gray-300 text-sm mb-3 group-hover:text-white transition-colors duration-500">{method.description}</p>
            ...
            <div className="flex space-x-6">
              {footerSocials.map((social, index) => (
                <a 
                  ...
                  className={`group relative w-16 h-16 bg-gradient-to-br from-cyber-cyan/20 to-blue-500/20 rounded-2xl flex items-center justify-center transition-all duration-700 hover:scale-125 hover:rotate-12 backdrop-blur-sm border border-cyber-cyan/30 hover:border-cyber-cyan/60 hover:shadow-2xl hover:shadow-cyber-cyan/40 ${social.color}`}
                  style={{ animationDelay: `${index * 0.1}s` }}
                >
                  <social.Icon size={24} className="transition-all duration-500 group-hover:scale-110" />
                  
                  {/* Hover background effect */}
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-cyber-cyan/30 to-blue-500/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
